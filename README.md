...
| **[proofscan](https://github.com/xnasusx/proofscan)** `TypeScript` | Layered application flaw scanner. Deterministic AST rules that model the Express route inventory — middleware chains, mount paths, registration order — to find authorisation, fallback-secret, CORS, rate-limit and schema-drift defects, alongside Semgrep, Gitleaks and Trivy. Every report leads with per-scanner coverage, because a scanner that never ran looks exactly like a clean result. Command-line tool, so nothing deployed. |
...
- Writing at [medium.com/@xnasusx](https://medium.com/@xnasusx) — including *"Hari Seldon Would've Made a Great CISO: What Cyber Risk Analysts Can Learn From Asimov's Foundation."*
